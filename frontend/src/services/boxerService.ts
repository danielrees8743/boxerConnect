...
export const boxerService = {
  ...
  async createBoxer(data: CreateBoxerData): Promise<BoxerProfile> {
    const response = await apiClient.post<ApiResponse<BoxerProfile>>('/boxers', data);
  ...
  async getMyBoxer(): Promise<BoxerProfile> {
    const response = await apiClient.get<ApiResponse<BoxerProfile>>('/boxers/me');
    if (!response.data.data) {
  ...
  async getBoxer(id: string): Promise<BoxerProfile> {
    const response = await apiClient.get<ApiResponse<BoxerProfile>>(`/boxers/${id}`);
    if (!response.data.data) {
  ...
  async updateBoxer(id: string, data: UpdateBoxerData): Promise<BoxerProfile> {
    const response = await apiClient.patch<ApiResponse<BoxerProfile>>(`/boxers/${id}`, data);
    if (!response.data.data) {
